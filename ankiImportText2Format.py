print("Check formatting of input.txt if you haven't. Type 'y' to proceed")
if input() != 'y':
    print("you typed not 'y' - aborting script")
    exit()

print("type tags, below is example")
print('биология биология_иммунология')
tags = input()
...
# make list with lines for final file
listForNewFile = []
for number in idsOfStartingLines:
    ...
    while lines[number + 2] != '\n':
        listForNewFile[-1] = listForNewFile[-1] + '<br><br>' + lines[number + 2]
        number = number + 2
    listForNewFile[-1] = listForNewFile[-1] + ';' + tags
