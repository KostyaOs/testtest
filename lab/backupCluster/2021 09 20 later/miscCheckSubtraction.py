...
# calculate thresholds IN SIGMAS that are NOT based on median:
    ...
    for norm in norms:
        ...
        if len(cases) != 0:
            ...
            for case in cases:
                ...
                # get differences, 3 strings below are correct code

                result = a['balanced'] - b['balanced']
                result *= 2
                result /= (a['balanced'] + b['balanced'])
...
with open('output/finalThresholds.csv', 'wt') as data_file:
    data_file.write(',chorion,fibroblast')
    data_file.write('\n')
    data_file.write('female,')
    data_file.write(f'{finalThresholds[0]},')
    data_file.write(f'{finalThresholds[1]},')
    data_file.write('\n')
    data_file.write('male,')
    data_file.write(f'{finalThresholds[2]},')

###########
# OPTIONS #
###########

if choice == 'calculate outliers for 3 groups based on median':
    ...
    for group in groups:
        ...
        if sex == 'male':
            sex = 1
            threshold = thresholds['chorion'][sex]
