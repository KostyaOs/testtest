...
# receive input
file = sys.argv[1]
balancing = sys.argv[2]
automatic_scaling = sys.argv[3]
if len(sys.argv) == 6:
    input_vmin = sys.argv[4]
    input_vmax = sys.argv[5]
...
if balancing == 'y':
    ...
    A = clr.matrix(balance=False)[:,:]  # get matrix of raw values
...
if balancing == 'y':
    ...
    value_column = 'count'
    extra_column = 'balanced'
    mtx = clr.matrix(balance=False)[:]
    mtx[~np.isfinite(mtx)] = 0  # change non-numbers to be zeros
    norm = mtx.sum()
...
output_path = 'output/' + file[file.find('/'):file.find('.cool')] + '.png'
