...
def mainfunc(inpathF, filenameF, outpathF):
    clr = cooler.Cooler(inpathF + '/' + filenameF)
    ...
    mtx['balanced'] = mtx['balanced'].div(mtx['balanced'].sum())  # apply norm to contacts = total trans normalization
    ...
    mtx.to_csv(outpathF + '/' + key(filenameF) + ',transcontacts.csv', index=False, index_label=False)
