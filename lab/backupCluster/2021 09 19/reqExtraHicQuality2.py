...
for string in lines:
    if str1(string) == '0' and str2(string) == '0':  # ff
        realHic += 1
        if chr1(string) != chr2(string):    # trans
            fftrans += 1
    elif str1(string) != '0' and str2(string) != '0':  # rr
        realHic += 1
        if chr1(string) != chr2(string):    # trans
            rrtrans += 1

realTrans = fftrans + rrtrans
realCis = realHic - realTrans
transcis = realTrans / realCis

specimen = sys.argv[2]
with open('/mnt/storage/home/knostroverkhii/workspace/output/' + specimen + '.txt', 'wt') as data_file:
    data_file.write(f'{transcis}')


if choice == 'DEinIntra, realDE, rings':
    file = sys.argv[1]
    with open(file, "rt", encoding="utf-8") as f:
        lines = f.readlines()
    total = len(lines)
